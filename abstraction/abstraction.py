...
class Abstraction:
    ...
    def gen_abs_state(self, route_size, route_res, speed_range, speed_res):
        ...
        # Velocity dimension
        v_bl = 0
        v_bu = int(speed_range / speed_res)  
    ...
    def trans_func(self, state, action):
        def get_move_transitions(move_action):
            ...
            if move_action == 'l': 
                ...
                prob_spatial[3, 3] = 0.5  # r+1, ey+1
                # Some uncertainty around the main transition
                prob_spatial[3, 2] = 0.25  # r+1, ey+0 (forward only)
                prob_spatial[2, 3] = 0.25  # r+0, ey+1 (left only)
            ...
            for j in range(5):  # δey changes
                for k in range(5):  # δv changes
                    if prob_spatial[i, j] > 0 and prob_v[k] > 0:
                        ...
                        # Check bounds
                        r_bl = 0
                        r_bu = self.map_shape[0] - 1
                        ey_bl = 0
                        ey_bu = self.map_shape[1] - 1
                        v_bl = 0
                        v_bu = self.map_shape[2] - 1
