...
# Todo: 1) plot risk field 2) local_perception 4) recursive feasibility
...
def main():
    ...
    static_label = {(5, 15, 5, 15): "c",
                    (0, 5, 10, 15): "t",
                    (0, 5, 0, 5): "o",
                    (0, 5, 15, 20): "o"}
    # cost_func = {"safe": 100, "co_safe": 0}
    cost_func = {"c": 5, "v0": 10, "v1": 2, "o": 10}
    ...
    ego_state = np.array([12.5, 2.5, np.pi / 2])
    ego_pos = ego_state[:2]
    ego_prod_state = (0, 1, 1)
    abs_state_sys = [-1, -1]
    abs_state_env = 1

    oppo_car_state = np.array([7.5, 18, -np.pi / 2])
    oppo_car_pos = oppo_car_state[:2]

    label_func = dyn_labelling(static_label, oppo_car_pos, [0, -1])
