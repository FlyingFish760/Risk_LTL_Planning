...
def main():
    ...
    target_state = [abs_model.get_state_index([32, 60]), abs_model.get_state_index([28, 60])]
    ...
    plt.axis('off')
    plt.axis('equal')
    ax.plot_surface(X, Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False, alpha=0.9)
    ...
    for i in range(12):
        ...
        ax.quiver(traj_pos[0], traj_pos[1], 0.1, action_arrow[0], action_arrow[1], 0, arrow_length_ratio=0.15)
        plt.plot(traj_pos[0], traj_pos[1], 0.1, 'o', color='r', markersize=1)
    for obs in square_obs_list:
       ax.bar3d(obs[0] + 32, obs[1], 0.0, obs[2], obs[3], 0.05, color = 'grey', alpha=1)
